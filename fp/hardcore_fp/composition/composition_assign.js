...
const isLastInStock = _.compose(_.last, _.map(_.prop("in_stock")));
...
const nameOfFirstCar = _.compose(_.head, _.map(_.prop("name")));
...
const availablePricesArray = _.compose(_.filter(_.prop("in_stock")));

const availablePrices = _.compose(
  _.join(", "),
  _.map(formatDollarValue),
  _.map(_.prop("in_stock"))
);
